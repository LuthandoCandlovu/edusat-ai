"""
🚀 EDUSAT AI - Professional ML Pipeline (FIXED)
Eastern Cape Dropout Risk Prediction
Author: Your Name
Version: 1.0.0
"""
import pandas as pd
...
REPORT_OUT = f"reports/model_report_{datetime.now().strftime('%Y%m%d')}.txt"
...
def load_and_prepare_data():
    ...
    print("📂 Loading Eastern Cape dataset...")
    
    # Check if file exists
    if not os.path.exists(DATA_PATH):
        # Try alternative path
        alt_path = "Eastern_Cape_Education_Factors_Dataset.xlsx"
        if os.path.exists(alt_path):
            df = pd.read_excel(alt_path, sheet_name="data")
        else:
            raise FileNotFoundError(f"❌ Could not find data file at {DATA_PATH} or {alt_path}")
    else:
        df = pd.read_excel(DATA_PATH, sheet_name="data")
    
    print(f"✅ Loaded {len(df)} learners, {len(df.columns)} features")
    
    # Create binary target (for ROC-AUC)
    df['risk_binary'] = (df[TARGET] == 'At Risk').astype(int)
    
    # Remove ID column if present
    drop_cols = [c for c in ["LearnerID", TARGET] if c in df.columns]
    X = df.drop(columns=drop_cols)
    y = df[TARGET].astype(str)  # For classification
    y_binary = df['risk_binary']  # For ROC-AUC
    
    return X, y, y_binary, df

print("="*60)
print("🚀 EDUSAT AI - PROFESSIONAL MODEL TRAINING")
print("="*60)

X, y, y_binary, df_raw = load_and_prepare_data()
...
if num_cols:
    print(f"   Numeric example: {num_cols[:5]}")
if cat_cols:
    print(f"   Categorical example: {cat_cols[:5]}")
...
# Split data (using original y for stratification)
...
# ============================================================================
# CROSS-VALIDATION (Academic Rigor)
# ============================================================================
...
# Get probabilities for ROC-AUC (binary)
# Find which class is "At Risk"
...
# ============================================================================
# FEATURE IMPORTANCE (Policy Insights)
# ============================================================================
...
try:
    ...
    feat_imp = pd.DataFrame({'feature': ['Unknown'], 'importance': [1.0]})

# ============================================================================
# BIAS CHECK (Ethical AI)
# ============================================================================
print("\n⚖️ Running bias checks...")

# Create test dataframe with predictions
X_test_with_pred = X_test.copy()
X_test_with_pred['Actual'] = y_test.values if hasattr(y_test, 'values') else y_test
...
if 'Gender' in X_test_with_pred.columns:
    ...
    gender_results = X_test_with_pred.groupby('Gender').agg({
        ...
        'Predicted': lambda x: (x == 'At Risk').mean() if hasattr(x, 'iloc') else 0
    })
...
# Check rural/urban bias
if 'RuralStatus' in X_test_with_pred.columns:
    print("\n📊 Rural/Urban analysis:")
    rural_results = X_test_with_pred.groupby('RuralStatus').agg({
        'Risk_Probability': 'mean',
        'Predicted': lambda x: (x == 'At Risk').mean() if hasattr(x, 'iloc') else 0
    })
    print(rural_results)
...
with open(REPORT_OUT, 'w') as f:
    f.write("="*60 + "\n")
    f.write("EDUSAT AI - MODEL TRAINING REPORT\n")
    f.write("="*60 + "\n\n")
    f.write(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n")
    f.write(f"Model Version: {MODEL_VERSION}\n")
    f.write(f"Training Samples: {len(X_train)}\n")
    f.write(f"Test Samples: {len(X_test)}\n")
    f.write(f"Features: {len(num_cols) + len(cat_cols)}\n\n")
    
    f.write("CROSS-VALIDATION:\n")
    f.write(f"F1 Scores: {cv_scores}\n")
    f.write(f"Mean F1: {cv_scores.mean():.3f}\n\n")
...
print(f"✅ Confusion Matrix: reports/confusion_matrix.png")
...
if 'feat_imp' in locals() and len(feat_imp) > 0:
    print(f"   • Top risk factor: {feat_imp.iloc[0]['feature']}")
print("="*60)

# Optional: Show feature importance plot if in interactive environment
try:
    if len(feat_imp) > 0:
        plt.figure(figsize=(10, 6))
